...
class OeMedicalAppointment(osv.Model):
    ...
    def onchange_date(self, cr, uid, ids, appointment_day, appointment_hour, appointment_minute, context=None):
        if appointment_day == False:
            appointment_day = ''
        if appointment_hour == False:
            appointment_hour = '00'
        if appointment_minute == False:
            appointment_minute = '00'

        res = {}
        res = {'value': { 
                    'appointment_date': appointment_day + ' ' + appointment_hour + ':' + appointment_minute + ':00'
                    } }
        return res

    # def _get_ssn_from_name(self, cr, uid, ids, field_name, arg, context):
    #     res = {}
    #     ssn = ''
    ...
    def _get_ssn(self, cr, uid, ids, field_name, arg, context=None):
        res = {}
        for obj in self.browse(cr, uid, ids, context=context):
            res[obj.id] = obj.patient_id.ssn or 'Not Specified'
        return res
    ...
    _columns = {
        ...
        # 'alias': fields.char(size=256, string='SSN', ), #should be concluded from name
        'alias': fields.function(_get_ssn, type='char', string='SSN', help="Social Security Number", multi=False),
